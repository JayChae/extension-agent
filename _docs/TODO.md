...
> **방식(기본값): 독립 측정 스크립트.** 익스텐션·백엔드 없이 작은 스크립트로 측정만 한다. 가볍고 빠르며, 이때 짠 "요소 인덱싱" 로직이 그대로 Phase 2의 content script로 재활용된다.
...
- [ ] (부산물) 한 화면 인덱싱+왕복에 대략 몇 초 걸리는지 감 잡기 → Phase 3 지연 예산의 기준값
...
## Phase 1 — 뼈대: 메시지가 한 바퀴 돈다 (걷기 전에 서기) (§2, §12)

> 두뇌·학습 다 빼고, **"버튼 누르면 → 백엔드 갔다 → 화면에 돌아온다"** 배관만 먼저 통한다.

- [ ] 레포 구조 + 백엔드 FastAPI 프로젝트(`pydantic-ai-slim[anthropic]` 의존성)
- [ ] WebSocket 1개 엔드포인트 (사이드패널 ↔ 백엔드 양방향)
- [ ] 익스텐션 MV3 골격: `sidePanel` + 얇은 SW(툴바클릭→패널오픈, 라우팅만) + content script(`*.scourt.go.kr`, `all_frames:true`)
...
## Phase 3 — 두뇌: 단일 에이전트 루프 (§1, §5, §6 가드레일)
...
## Phase 5 — 🟢 가르치면 기억한다 ("Show me" → SOP) (§7 경로①, §9)
...
## Phase 6 — 🟢 교정으로 배운다 (레슨 누적 + 화해) (§7 경로②③)
...
## 🔒 횡단 안전 (Phase 1부터 깔고, MVP 마감 전 점검) (§4, §11, §13-8)
...
> ① 막히면 묻고(P4) · ② 가르치면 기억하고(P5) · ③ 교정으로 배우고(P6) · ④ 점점 나아진다(P7). + 횡단 안전 최소선 켜짐.
