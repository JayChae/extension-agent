...
## 📍 현재 위치
> **Phase 0 시작 전.** 레포에 코드 없음(문서 4개뿐). 아래 Phase 0부터.

---

## Phase 0 — 그라운딩 스파이크 (코드 본격 착수 *전*, 0순위) (§13, §3)

> **이 시스템 전체가 "scourt 화면에서 버튼·입력칸을 정확히 집어내기"에 달려 있다.** 이게 안 되면 나머지를 아무리 잘 짜도 에이전트가 손을 못 쓴다. 그래서 제일 먼저, 제일 약한 고리를 찍어본다.
> **방식(기본값): 브라우저 콘솔 스니펫.** 레포·빌드 없이 scourt 페이지 콘솔에 붙여넣어 측정만 한다. 가장 가볍고, 이때 짠 "요소 인덱싱" 로직이 그대로 Phase 3의 content script로 재활용된다.

- [ ] scourt 대표 화면 3~4개 선정 (로그인 / 사건검색 / 결과표 / 문서열람)
- [ ] §3의 멀티시그널 휴리스틱으로 인터랙티브 요소 인덱싱 (태그+클릭핸들러+ARIA+cursor:pointer+키워드)
- [ ] 각 화면에서 "사람이 실제로 누르는 핵심 컨트롤"을 손으로 세고, 스크립트가 몇 개 잡았는지 대조
- [ ] (부산물) 합성 이벤트 click이 익스텐션 환경에서도 먹히는지 1건 확인 — 운영 RPA로 확인됐으나(§15) 환경이 다름
- [ ] (부산물) 한 화면 인덱싱+왕복에 대략 몇 초 걸리는지 감 잡기 → Phase 4 지연 예산의 기준값

**✅ 검증:** 핵심 컨트롤 포착률 측정됨.
- ≥ 90% → Phase 1로 진행
- ⛔ ≤ 70% → **멈춤.** §3 지각 설계부터 손본다(문서 다듬기보다 우선). MVP 보류.
...
> Phase 0의 인덱싱 로직을 content script로 정식 이식하고, 백엔드가 시키는 대로 실제 DOM을 조작한다.

- [ ] content script: 매 스텝 컴팩트 인덱스 요소 목록 생성 (Phase 0 코드 재활용)
