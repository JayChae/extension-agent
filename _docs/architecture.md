...
> 대상 사용자 예: 송무팀이 대한민국 법원 사이트(scourt.go.kr) 업무를 가르쳐 자동화.
...
```
┌─────────────────────────────────────────────────────────────────┐
│  브라우저 (송무 직원 PC) — scourt.go.kr 탭 / 팝업 / iframe        │
...
- **인터랙티브 판정은 태그명이 아니라 멀티시그널 휴리스틱**: 네이티브 태그 + 클릭 핸들러 + ARIA 역할 + `aria-expanded/pressed` + `cursor:pointer` + 키워드 속성. scourt.go.kr은 태그만으로 버튼 절반을 놓치는 커스텀 JS 위젯 정부 사이트라 필수.
...
- ⚠️ 공동인증서 모듈이 DOM이 아니라 **OS 네이티브 창**으로 뜨면 익스텐션이 못 봄 → 별도 **네이티브 헬퍼(동반 데스크톱 앱)** 필요. scourt.go.kr이 DOM/네이티브 중 무엇인지 확인 필요(섹션 15).
