...
- **네이티브 JS 다이얼로그(`alert`/`confirm`) 자동 처리:** scourt는 네이티브 경고창을 띄움(기존 RPA가 `switch_to.alert`로 처리). content script(isolated world)는 이 창을 못 닫으므로, 페이지 스크립트 실행 전 **MAIN world에 주입한 작은 훅으로 `window.alert/confirm`을 가로채** 자동 수락 + 메시지를 모델 관측으로 전달. "content script는 얇게(isolated)" 원칙의 *유일한* MAIN-world 예외. **Phase 9B 구현:** `extension/main_hook.js`(manifest 둘째 `content_scripts` 엔트리 `world:"MAIN" run_at:"document_start"`)가 `alert`(자동닫힘)·`confirm`(자동수락 `true`)·`prompt`(자동취소 `null`)를 덮어쓰고 `__ai_newbie_dialog__` CustomEvent(detail=문자열, 월드 간 클론 안전)로 ISOLATED content.js에 전달 → content가 버퍼(stripMarkers 새니타이즈)해 다음 관측에 `dialogs`로 싣고 `render_observation`이 펜스 안에 노출. 비가역 클릭은 9A 크리티컬 게이트가 먼저 사람 승인을 받으므로 confirm 자동수락은 안전.
...
- 비밀값(ID·비번·**인증서 PIN**)은 **백엔드 금고(암호화)에 1회 등록** → 실행 때 harness가 꺼내 `fill_credential`로 직접 입력하고, **Claude는 값을 절대 못 봄**(관측·로그에서 마스킹). 비밀이 모델·SOP·평문 어디에도 안 남음.
  - **Phase 9B 구현:** `backend/vault.py` — `cryptography.fernet`(대칭 암호화), 허용 kind 고정집합 `(login_id, login_pw, cert_pin)`. 마스터키는 **자동 생성·재사용**(`backend/vault/master.key`, env `VAULT_KEY` 있으면 우선 — 운영 주입용), 암호문은 `backend/vault/secrets.json`. 둘 다 git 미추적(키·암호문 colocated는 로컬 단일사용자 백엔드라 수용 — .env도 같은 호스트). **등록은 사이드패널 '자격증명' 카드 1회**(사용자 확정: 비전문가 편의 — 터미널 대신; `register_credential` WS → `vault.put`, password 입력칸). 값은 로컬 WS(`127.0.0.1`)로 백엔드까지만 가고 모델·관측·로그·UI엔 안 남음. `fill_credential(index, kind)` 도구는 모델이 `kind`만 지정 → harness가 금고에서 꺼내 `type` 액션(`secret:true`)으로 직접 입력. 누출 방어: `session.act`는 `text`를 감사로그·모델에 미기록(§3 구조적 블랭킹), content.js는 `secret`이면 관측 note에 입력값을 echo 안 함(`(비밀값)`), 사이드패널은 command를 화면에 표시 안 함. 키 없음/복호화 실패는 `None`(안전 폴백).
